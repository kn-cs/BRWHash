...
def getKeyRandom(k):
	...
	key = []
	f = open("brwhash1305_t5_speed_record_test_cases_keyfile.txt", "w")
	for i in range(k): 
...
def genSpeedTestCases():
	...
	t = 5;
	f = open("brwhash1305_t5_speed_record_test_cases.txt", "w")
	start = 128
	stop = 25600+128
	step = 128
...
def genSpeedTestCasesDirectCmp():

	key = getKeyRandom(k)
	keyFldElt = getKeyFldElt(key,k)
    	
	t = 5;
	f = open("brwhash1305_t5_speed_record_test_cases_direct_cmp.txt", "w")
	lst = [80, 400, 800, 4000, 8000, 16000, 24000, 32000, 40000, 48000, 56000, 64000]
	for msgLen in lst:	
		for i in range(1,2):
			msg = getRandomString(msgLen)
			msgEltsLst = []
