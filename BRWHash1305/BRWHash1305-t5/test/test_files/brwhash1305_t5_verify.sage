...
def getKeyRandom(k):
	...
	key = []
	f = open("brwhash1305_t5_verify_keyfile.txt", "w")
	for i in range(k): 
...
def genTestCases():
	...
	t = 5;
	f1 = open("brwhash1305_t5_verify_test_cases.txt", "w")
	f2 = open("brwhash1305_t5_verify_s_outputs.txt", "w")	
	start = 1
	stop = 2048
	step = 1
	for msgLen in range(start,stop,step):	
		for i in range(1,2):
			...
			f1.write("\n")
			digest = ComputeBRW(t,keyFldElt,msgEltsLst)
